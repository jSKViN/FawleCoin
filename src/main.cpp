...
#if defined(NDEBUG)
# error "FawleCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd4ab6ee9c6fca56047fdaf5c166b2a4d2abf39136b6c2d9dd361d7afd400e74b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FawleCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FawleCoin Signed Message:\n";
...
{
    // FawleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 43400000 * COIN;
            }

    // Subsidy is cut in half every 186000 blocks
    nSubsidy >>= (nHeight / 186000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1600 * 60;
static const int64 nTargetSpacing = 800 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FawleCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FawleCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1463130905, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Why We Need More Resources for the Prescription Opioid and Heroin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a2edc33557c723cbd167128650c61472cd684de013016bf076b2f647801bedace82816b658a608e3e272b9d90fccbddc81b7958c24f15e36ed44c14aec80df7f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1463130905;
        ...
        assert(block.hashMerkleRoot == uint256("0x0b4d32686f7f214c5b11584da6bb4496741431236ccdab9b5d2a3908e6a72a1a"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0xaf, 0xda, 0xad }; // FawleCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FawleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FawleCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FawleCoinMiner(CWallet *pwallet)
{
    printf("FawleCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fawlecoin-miner");
    ...
    try { loop {
        ...
        printf("Running FawleCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FawleCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FawleCoinMiner, pwallet));
}
